  ...
  return (
    <footer className="bg-primary w-full pt-16">
        <div className="max-container flex w-full flex-col">
          <div className="flex items-start justify-between gap-8 mb-16">
            {FOOTER_CONTENTS.map((content) => (
              <FooterColumn key={content.title} title={content.title}>
                <ul className="flex flex-col text-base font-normal gap-2">
          ...
          <p className="mt-6 py-3 w-full text-center text-sm font-semibold rounded-t-full text-black bg-white">Copyright © 2024 by <Link className="hover:underline" href="https://dacotours.com/">DACOTOURS</Link>.</p>
        </div>
    </footer>
  )
}
...
const FooterColumn = ({ title, children }) => {
  return (
    <div className="flex flex-col gap-6 text-black w-1/4">
